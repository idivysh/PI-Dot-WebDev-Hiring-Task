...
const Stars = () => {
  ...
  const stars = React.useMemo(() => {
    return [...Array(80)].map(() => {
      ...
      const targetTop = `calc(${random() * 100}% + ${randomMove()}px)`;
      const targetLeft = `calc(${random() * 100}% + ${randomMove()}px)`;
      const opacity = randomOpacity();
      const duration = random() * 10 + 20;
      return { initialTop, initialLeft, targetTop, targetLeft, opacity, duration };
    });
  }, []);
